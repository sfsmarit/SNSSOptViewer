...
namespace SNSSOptViewer
{
    ...
    {
        // Default root folder (initial value only)
        private const string DefaultRootFolder = "";

        // Current scan root (user switches via drag & drop)
        private string _scanRoot = DefaultRootFolder;

        // --- Top bar host (2 rows) ---
        private TableLayoutPanel _root;       // whole form
        private TableLayoutPanel _topRows;    // two-row top area

        // Row-1 (Refresh + Folder)
        private FlowLayoutPanel _barRow1;
        private Button _btnRefresh;    // rescan
        private Label _lblFolderPath;  // current folder (ellipsis, tooltip=full path)
        private Label _lblDropHint;    // "(Drag & drop a folder here)" shown only when folder is not set

        // Row-2 (Solution + FileName)
        private FlowLayoutPanel _barRow2;
        private ComboBox _cmbPairs;    // solution base selector (paired files)
        private Label _lblPaths;       // current file name (ellipsis, tooltip=full path)
        ...
        // Current paths for label update
        ...
        {
            // Root layout: [Top (auto)] / [Tabs (*))]
            _root = new TableLayoutPanel { Dock = DockStyle.Fill, ColumnCount = 1, RowCount = 2 };
            _root.RowStyles.Add(new RowStyle(SizeType.Absolute, 70)); // two lines ~ 70px
            _root.RowStyles.Add(new RowStyle(SizeType.Percent, 100));
            Controls.Add(_root);

            // --- Top area: 2 rows ---
            _topRows = new TableLayoutPanel { Dock = DockStyle.Fill, ColumnCount = 1, RowCount = 2, Padding = new Padding(8, 6, 8, 4) };
            _topRows.RowStyles.Add(new RowStyle(SizeType.Absolute, 32)); // Row-1: Refresh + Folder
            _topRows.RowStyles.Add(new RowStyle(SizeType.Absolute, 32)); // Row-2: Solution + FileName
            _root.Controls.Add(_topRows, 0, 0);

            _tip = new ToolTip();

            // --------------- Row-1: Refresh (left) + Folder (right) ---------------
            ...
            // Drag & drop enable (row1 + whole form)
            ...
            _lblDropHint = new Label
            {
                ...
                Visible = false // default hidden; shown only when folder is not set/valid
            };
            ...
            _cmbPairs.SelectedIndexChanged += (s, e) => LoadSelectedPair();
            ...
            _barRow2.Controls.AddRange(new Control[] { _cmbPairs, _lblPaths });
        ...
        {
            // Row-1 width for folder label or hint (right of Refresh)
            int margin = 24;
            int w1 = _barRow1.ClientSize.Width - _btnRefresh.Width - margin;
            if (w1 < 180) w1 = 180;

            if (_lblFolderPath.Visible)
            {
                _lblFolderPath.Width = w1;
            }
            if (_lblDropHint.Visible)
            {
                // when hint is visible, let it take the remaining width (no ellipsis needed)
                // nothing to set (AutoSize=true), but ensure folder label doesn't steal width
                _lblFolderPath.Width = 0;
            }

            // Row-2 width for file name label (right of combo)
            int w2 = _barRow2.ClientSize.Width - _cmbPairs.Width - margin;
            if (w2 < 180) w2 = 180;
            _lblPaths.Width = w2;
        }
        ...
        private void UpdateFolderLabel()
        {
            ...
            // Show hint only when folder is not set/valid
        ...
        // Drag & drop: accept folder (or file -> parent folder)
        ...
        private void PopulatePairs()
        {
            try
            {
                ...
                var stateFiles = Directory.EnumerateFiles(_scanRoot, "*_state(SA).xls", SearchOption.TopDirectoryOnly)
                    .Select(p => new FileInfo(p))
                    .OrderByDescending(fi => fi.LastWriteTime)
                    .ToList();

                var pairs = new List<PairItem>();
                foreach (var st in stateFiles)
                {
                    string baseName = Path.GetFileName(st.FullName);
                    if (baseName == null) continue;

                    string suffix = "_state(SA).xls";
                    if (!baseName.EndsWith(suffix, StringComparison.OrdinalIgnoreCase)) continue;
                    string baseNoSuffix = baseName.Substring(0, baseName.Length - suffix.Length);

                    string charPath = Path.Combine(st.DirectoryName, baseNoSuffix + "_char(SA).xls");
                    if (File.Exists(charPath))
                    {
                        pairs.Add(new PairItem
                        {
                            BaseName = baseNoSuffix,
                            StatePath = st.FullName,
                            CharPath = charPath,
                ...
                if (pairs.Count == 0)
                {
                    _cmbPairs.DataSource = null;
                    _cmbPairs.Items.Clear();
                    _cmbPairs.Items.Add("(no paired *_state(SA).xls & *_char(SA).xls found)");
                    _cmbPairs.SelectedIndex = 0;
                    _lblPaths.Text = "";
                    _tip.SetToolTip(_lblPaths, "");
                    return;
        ...
        {
            if (!(_cmbPairs.SelectedItem is PairItem it)) return;
            ...
            this.Text = $"SA Log Viewer (Variable & Goal)  —  {it.BaseName}";
        ...
        private class StateTabControl : UserControl
        {
            ...
            private TableLayoutPanel _rows;   // chart / ctrl(TopN) / ranking
            ...
            private void BuildUI()
            {
                ...
                // Chart with two areas
                _chart = new Chart { Dock = DockStyle.Fill };
                var main = new ChartArea("main");
                main.AxisX.Title = ""; // remove X label for Error/T
                ...
                // Legend inside main bottom-right with white background
                ...
                // Control row (TopN right-aligned)
            ...
            private static DataTable LoadAsNumericTable(string path)
            {
                ...
                string header = lines.First(l => !string.IsNullOrWhiteSpace(l)).Trim();
        ...
        // =====================================================================
        // Goal tab (char viewer) - Inputs group removed (flat top row)
        // =====================================================================
        private class CharTabControl : UserControl
        {
            private TableLayoutPanel root;

            private TableLayoutPanel tlp; // top controls (no GroupBox)
            ...
            private void BuildUI()
            {
                ...
                // Top controls (flat; no "Inputs")
                tlp = new TableLayoutPanel { Dock = DockStyle.Fill, ColumnCount = 8, RowCount = 1, Padding = new Padding(4, 4, 4, 4) };
                for (int i = 0; i < 8; i++) tlp.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 12.5f));
                tlp.RowStyles.Add(new RowStyle(SizeType.Percent, 100f));
                root.Controls.Add(tlp, 0, 0);

                cmbSolution = new ComboBox { DropDownStyle = ComboBoxStyle.DropDownList, Anchor = AnchorStyles.Left, Width = 220 };
                cmbSolution.SelectedIndexChanged += (s, e) => ApplySelectedSolution();

                btnPrevSol = new Button { Text = "◀ Prev", Anchor = AnchorStyles.Left, Width = 64 };
                btnPrevSol.Click += (s, e) => { int i = cmbSolution.SelectedIndex - 1; if (i >= 0) cmbSolution.SelectedIndex = i; };
                btnNextSol = new Button { Text = "Next ▶", Anchor = AnchorStyles.Left, Width = 64 };
                btnNextSol.Click += (s, e) => { int i = cmbSolution.SelectedIndex + 1; if (i < cmbSolution.Items.Count) cmbSolution.SelectedIndex = i; };

                lblTopN = new Label { Text = "TopN", Anchor = AnchorStyles.Left, AutoSize = true };
                numTopN = new NumericUpDown { Minimum = 1, Maximum = 9999, Value = 24, Width = 72, Anchor = AnchorStyles.Left };
                numTopN.ValueChanged += (s, e) => ApplyTopNOnly();

                tlp.Controls.Add(cmbSolution, 0, 0);
                tlp.Controls.Add(btnPrevSol, 1, 0);
                tlp.Controls.Add(btnNextSol, 2, 0);
                tlp.Controls.Add(lblTopN, 3, 0);
                tlp.Controls.Add(numTopN, 4, 0);
                ...
                var series = new Series("ERR") { ChartType = SeriesChartType.Column, XValueType = ChartValueType.String, YValueType = ChartValueType.Double };
            ...
            public void LoadCharFile(string charPath)
            {
                try
                {
                    ...
                    if (string.IsNullOrWhiteSpace(_currentFile) || !File.Exists(_currentFile)) return;

                    _lines = File.ReadAllLines(_currentFile);
                    if (_lines.Length == 0) return;

                    _headerRow = -1;
                    for (int i = 0; i < _lines.Length; i++) if ((_lines[i] ?? "").Contains("ERR_")) _headerRow = i;
                    ...
                    _errCols = headers.Select((h, i) => new ErrCol { Index = i, Name = h })
                                      .Where(x => (x.Name ?? "").StartsWith("ERR_", StringComparison.Ordinal))
                                      .ToList();
                    if (_errCols.Count == 0) return;
                    ...
                    for (int r = firstDataRow; r < _lines.Length; r++)
                    {
                        ...
                        var m = rx.Match(token.Trim().Trim('"', '\''));
                    ...
                    cmbSolution.DataSource = _solutions.Select(s => new ComboItem { Display = $"{s.Token}", Number = s.Number, RowIndex = s.RowIndex }).ToList();
                    cmbSolution.DisplayMember = nameof(ComboItem.Display); cmbSolution.ValueMember = nameof(ComboItem.Number);
            ...
            {
                if (cmbSolution.SelectedItem == null || _errCols.Count == 0) return;
                var item = (ComboItem)cmbSolution.SelectedItem;
                int rowIndex = item.RowIndex;
                if (rowIndex <= _headerRow || rowIndex >= _lines.Length) return;
                if (string.IsNullOrWhiteSpace(_lines[rowIndex])) return;
                ...
                foreach (var col in _errCols)
                {
                    ...
                    list.Add(new Row { Name = ShortLabel(col.Name), Value = value });
                }
            ...
            {
                if (_allRowsCurrent == null || _allRowsCurrent.Count == 0) return;
                ...
                for (int i = 0; i < slice.Count; i++) { slice[i].Rank = i + 1; slice[i].Share = (totalAll > 0) ? (slice[i].Value / totalAll) : 0.0; }
            ...
            {
                g.DataSource = rows.Select(x => new Row { Rank = x.Rank, Name = x.Name, Value = x.Value, Share = x.Share }).ToList();
                var c = g.Columns[nameof(Row.Share)]; if (c != null) c.DefaultCellStyle.Format = "P1";
            }
            ...
            private static string ShortLabel(string full)
            {
                ...
                string s2 = full.StartsWith("ERR_", StringComparison.Ordinal) ? full.Substring(4) : full;
                int b = s2.IndexOf('[');
                if (b >= 0) s2 = s2.Substring(0, b);
                return s2.Trim();
            }
            ...
            private class ErrCol { public int Index { get; set; } public string Name { get; set; } }
            private class Row { public int Rank { get; set; } public string Name { get; set; } public double Value { get; set; } public double Share { get; set; } }
            private class SolutionEntry { public int Number { get; set; } public string Token { get; set; } public int RowIndex { get; set; } }
